...
namespace CS422
{
    ...
    //private static string _rootPath = "/home/conner/Documents/422/422Repo/hw9/build/FSROOT";
    private static FileSys422 _fileSystem;
    ...
    public static void Service(WebRequest request)
    {
      ...
      {
        Console.WriteLine("Servic(): parsed[i]: " + s);
      }

      string requestedService = "/" + parsed[1];
      Console.WriteLine("Servic(): requestedService: " + requestedService);

      foreach (WebService service in _webServices)
      {
        Console.WriteLine("service.ServiceURI: "+ service.ServiceURI );
        //request.PrintRequest();
        if (service.ServiceURI == requestedService)
        {
